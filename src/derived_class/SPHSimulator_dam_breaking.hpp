...
{
public:
	SPHSimulator_dam_breaking(int N,  Real uParticle_len, Real dt, Real eta, Real B, Real alpha, Real rest_density, int with_viscosity, int with_XSPH, int solver_type)
	...
	virtual void generate_particles() override
    {
            ...
            test_cuboid.x_n = N+6;
            test_cuboid.y_n = static_cast<int>(sqrt(N)*N)+6;
            test_cuboid.z_n = 3*N;
