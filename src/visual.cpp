...
namespace Simulator
{
    ...
    Visualization::Visualization(std::string simfile, float shift_x)
    {
        player_init();
        simulation_init(simfile);
        render_mesh_flag = false;
        no_mesh = true;

        shift = Vector3f(shift_x, 0.0f, 0.0f);
    }

    Visualization::Visualization(std::string simfile, std::string meshfile, float shift_x)
    {
        player_init();
        simulation_init(simfile);
        mesh_init(meshfile);
    ...
    void Visualization::mesh_init(std::string meshfile)
    {
    ...
    void Visualization::simulation_init(std::string simfile)
    {
        ...
        if (total_frame_num < global_total_frame)
            global_total_frame = total_frame_num;
        eta = sim_rec.eta;
    ...
    void Visualization::player_init()
    {
        sim_count = 0;
        speed_ratio = 1.0;
        playback_flag = false;
	...
	*/
		...
		if (render_particle_flag)
		{
			...
			for (size_t i = 0; i < particles.size(); ++i)
			{
				...
				if(render_density_flag == render_velocity_flag) //if we set both rendering, we see it as no rendering
				{
                    ...
                    {
                        if (sets[i])
					        frame.draw_particle(Particle(p+shift).with_radius(particle_size*0.5).with_color(Color(0.0f, 0.5f, 1.0f)));
                        else 
                            frame.draw_particle(Particle(p+shift).with_radius(particle_size*0.5).with_color(Color(1.0f, 0.5f, 0.0f)));
                    } else {
					    frame.draw_particle(Particle(p+shift).with_radius(particle_size*0.5).with_color(Color(0.0f, 0.5f, 1.0f)));
                    }
				}
				else if(render_velocity_flag)
				{
					...
					float b = 1.0f - r;
					frame.draw_particle(Particle(p+shift).with_radius(particle_size*0.5).with_color(Color(r, 0.5f-r*0.5, b)));
				}
				else
				{
					//render density
        ...
        if (!no_mesh) // show mesh
        {
        	if (render_mesh_flag)
        	{
    			...
    			frame.draw(renderable(mesh)
    					   .with_position(0.0+shift[0], 0.0, 0.0)
        	...
        	if (render_bounding_box_flag)
        	{
            	...
            	// bottom
            	Line l0( shift + origin - Vector3f(bounding_box[0]/2.0, bounding_box[1]/2.0, 0.0), shift + origin - Vector3f(bounding_box[0]/2.0, -bounding_box[1]/2.0, 0.0), box_color);
            	Line l1( shift + origin - Vector3f(bounding_box[0]/2.0, bounding_box[1]/2.0, 0.0), shift + origin - Vector3f(-bounding_box[0]/2.0, bounding_box[1]/2.0, 0.0), box_color);
            	Line l2( shift + origin + Vector3f(bounding_box[0]/2.0, bounding_box[1]/2.0, 0.0), shift + origin + Vector3f(bounding_box[0]/2.0, -bounding_box[1]/2.0, 0.0), box_color);
            	Line l3( shift + origin + Vector3f(bounding_box[0]/2.0, bounding_box[1]/2.0, 0.0), shift + origin + Vector3f(-bounding_box[0]/2.0, bounding_box[1]/2.0, 0.0), box_color);

            	// top
            	Line l4( shift + origin - Vector3f(bounding_box[0]/2.0, bounding_box[1]/2.0, -bounding_box[2]), shift + origin - Vector3f(bounding_box[0]/2.0, -bounding_box[1]/2.0, -bounding_box[2]), box_color);
            	Line l5( shift + origin - Vector3f(bounding_box[0]/2.0, bounding_box[1]/2.0, -bounding_box[2]), shift + origin - Vector3f(-bounding_box[0]/2.0, bounding_box[1]/2.0, -bounding_box[2]), box_color);
            	Line l6( shift + origin + Vector3f(bounding_box[0]/2.0, bounding_box[1]/2.0, bounding_box[2]), shift + origin + Vector3f(bounding_box[0]/2.0, -bounding_box[1]/2.0, bounding_box[2]), box_color);
            	Line l7( shift + origin + Vector3f(bounding_box[0]/2.0, bounding_box[1]/2.0, bounding_box[2]), shift + origin + Vector3f(-bounding_box[0]/2.0, bounding_box[1]/2.0, bounding_box[2]), box_color);

            	// sides
            	Line l8( shift + origin - Vector3f(bounding_box[0]/2.0, bounding_box[1]/2.0, 0.0), shift + origin - Vector3f(bounding_box[0]/2.0, bounding_box[1]/2.0, -bounding_box[2]), box_color);
            	Line l9( shift + origin - Vector3f(bounding_box[0]/2.0, -bounding_box[1]/2.0, 0.0), shift + origin - Vector3f(bounding_box[0]/2.0, -bounding_box[1]/2.0, -bounding_box[2]), box_color);
            	Line l10( shift + origin + Vector3f(bounding_box[0]/2.0, -bounding_box[1]/2.0, 0.0), shift + origin + Vector3f(bounding_box[0]/2.0, -bounding_box[1]/2.0, bounding_box[2]), box_color);
            	Line l11( shift + origin + Vector3f(bounding_box[0]/2.0, bounding_box[1]/2.0, 0.0), shift + origin + Vector3f(bounding_box[0]/2.0, bounding_box[1]/2.0, bounding_box[2]), box_color);
    ...
    void Visualization::render_control_panel()
    {
        if (pausing_flag==false)
        {
            ...
            {
                sim_count = std::min(sim_count+render_step,global_total_frame-1);
            }
            else{
        ...
        if (ImGui::Begin("Control Panel"))
        {
            ...
            ImGui::SliderInt("current frame", &sim_count, 0, global_total_frame-1);
